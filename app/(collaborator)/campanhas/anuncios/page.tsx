...
export default function AnunciosPage() {
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      ...
      case 'planejamento':
        return { label: 'Planejamento', color: 'text-text-muted', icon: Clock, bg: 'bg-gray-400/10' }
      default:
        return { label: status, color: 'text-text-muted', icon: Clock, bg: 'bg-gray-400/10' }
    }
  }
  ...
  const CampaignCard = ({ campaign }: { campaign: AdCampaign }) => {
    ...
    return (
      <div className="bg-background border border-border p-6 rounded-xl group hover:border-emerald-500/30 transition-all shadow-sm">
        ...
        <div className="space-y-1 mb-6">
          <h3 className="text-base font-bold text-text-main leading-tight group-hover:text-emerald-400 transition-colors">
            {campaign.name}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border">
          <div>
            <p className="text-[10px] font-black text-text-disabled uppercase tracking-widest mb-1">Início</p>
            <p className="text-xs text-text-main">{formatDate(campaign.start_date)}</p>
          </div>
          <div>
            <p className="text-[10px] font-black text-text-disabled uppercase tracking-widest mb-1">Término</p>
            <p className="text-xs text-text-main">{formatDate(campaign.end_date)}</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="p-8 space-y-10">
      {/* Cabeçalho */}
      <div>
        <h1 className="text-2xl font-bold text-text-main">Campanhas de Anúncios</h1>
      ...
      {/* Loading */}
      {loading && (
        <div className="py-16 text-center text-text-muted italic">
      ...
      {!loading && noIntegration && (
        <div className="flex flex-col items-center justify-center py-16 gap-4 border-2 border-dashed border-border rounded-xl">
      ...
      {!loading && !noIntegration && (
        <>
          ...
          <section>
            ...
            {!integration?.meta_ads_connected ? (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-background border border-border text-text-muted text-sm">
              ...
              <div className="py-10 text-center text-text-muted italic border-2 border-dashed border-border rounded-xl">
