...
export default function ColaboradorChecklistsPage() {
  ...
  const ChecklistCard = ({ list }: { list: Checklist }) => {
    ...
    return (
      <div key={list.id} className={`bg-[#111] border rounded-xl p-5 flex flex-col gap-4 transition-all ${list.status === 'completed' ? 'border-emerald-500/30 opacity-80' : 'border-[#1a3a24]'}`}>
            ...
            {editingListId === list.id ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="bg-[#0a0f0c] border border-emerald-500/50 rounded px-2 py-1 text-sm text-white w-full"
                  autoFocus
                />
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3 group">
              <input
                ...
                className="w-4 h-4 rounded border-[#1a3a24] bg-transparent text-emerald-500 focus:ring-0 cursor-pointer"
              />
              ...
              {editingItemId === item.id ? (
                <div className="flex items-center gap-2 flex-1">
                  <input
                    ...
                    className="bg-[#0a0f0c] border border-emerald-500/50 rounded px-2 py-0.5 text-xs text-white w-full"
                    autoFocus
                  />
        ...
        <div className="flex items-center gap-2 mt-auto pt-2 border-t border-[#1a3a24]">
          <input
            type="text"
            ...
            className="flex-1 px-3 py-1.5 bg-[#0a0f0c] border border-[#1a3a24] rounded-lg text-xs focus:outline-none focus:border-emerald-500 transition-colors"
          />
          <button 
            onClick={() => handleAddItem(list.id, list.id)}
            className="p-1.5 bg-[#1a3a24] text-emerald-500 hover:text-white rounded-lg transition-colors"
          >
            <Plus size={14} />
          </button>
        </div>
  ...
  return (
    <div className="p-8 min-h-screen bg-[#0a0f0c] text-gray-100">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        ...
        <div className="flex flex-wrap items-center gap-3 bg-[#111] p-2 rounded-2xl border border-[#1a3a24]">
          <input
            type="text"
            value={newListTitle}
            ...
            className="px-4 py-2 bg-[#0a0f0c] border border-[#1a3a24] rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-colors min-w-[240px]"
          />
          <select 
            value={newListRecurrence}
            onChange={(e) => setNewListRecurrence(e.target.value as any)}
            className="px-3 py-2 bg-[#0a0f0c] border border-[#1a3a24] rounded-xl text-sm text-gray-300 outline-none focus:border-emerald-500"
          >
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {pendingLists.length === 0 && (
              <div className="col-span-full py-12 border-2 border-dashed border-[#1a3a24] rounded-2xl flex flex-col items-center justify-center text-gray-600">
