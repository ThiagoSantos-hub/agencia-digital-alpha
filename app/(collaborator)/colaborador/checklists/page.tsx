  ...
  return (
    <div className="p-8 min-h-screen bg-[#0a0f0c] text-gray-100">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight uppercase">Meus Checklists</h1>
          <p className="text-gray-500 text-sm mt-1 font-medium">Suas rotinas e verificações diárias.</p>
        </div>
      ...
      {isCreating && (
        <div className="mb-12 bg-[#111] border border-[#00ff88]/30 rounded-2xl p-6 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-black text-white uppercase">Novo Checklist Personalizado</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Nome do Checklist</label>
                <input
                  type="text"
                  value={newListTitle}
                  onChange={(e) => setNewListTitle(e.target.value)}
                  placeholder="Ex: Rotina de Atendimento, Postagens..."
              ...
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Dias da Semana (Reset Automático)</label>
                <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
                  {DIAS_SEMANA.map(day => (
                    <button
                      key={day.id}
                      onClick={() => toggleDay(day.id, newListDays, setNewListDays)}
                      className={`px-2 py-2.5 rounded-lg text-[10px] font-black uppercase transition-all border ${
                ...
                <p className="text-[10px] text-gray-600 mt-2 italic">* A lista resetará automaticamente nos dias selecionados.</p>
              </div>
            </div>

            <div className="space-y-4">
              <label className="block text-xs font-black text-gray-500 uppercase tracking-widest">Itens da Lista</label>
...
function ChecklistComponent({ 
  ...
  return (
    <div className={`bg-[#111] border rounded-2xl overflow-hidden transition-all duration-300 ${list.status === 'completed' ? 'border-[#00ff88]/20 opacity-60 hover:opacity-100' : 'border-[#1a3a24] hover:border-gray-700'}`}>
      <div className="p-5 border-b border-[#1a3a24] flex justify-between items-start bg-[#161616]">
              ...
              <div className="grid grid-cols-4 gap-1">
                {DIAS_SEMANA.map(day => (
                  <button
                    ...
                    className={`px-1 py-1 rounded text-[9px] font-black uppercase border ${days.includes(day.id) ? 'bg-[#00ff88] text-[#0a0f0c] border-[#00ff88]' : 'bg-[#0a0f0c] text-gray-500 border-[#1a3a24]'}`}
                  >
                    {day.label.substring(0, 3)}
                  </button>
                ))}
                ...
                {list.recurrence_days && list.recurrence_days.length > 0 ? (
                  list.recurrence_days.map(d => (
                    <span key={d} className="text-[9px] font-black text-[#00ff88] uppercase bg-[#00ff88]/10 px-1.5 py-0.5 rounded">
                      {DIAS_SEMANA.find(day => day.id === d)?.label.substring(0, 3)}
                    </span>
                  ))
                ) : (
      ...
      <div className="p-5 space-y-3">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3 group">
            <input
              ...
              className="w-5 h-5 rounded border-[#1a3a24] bg-[#0a0f0c] text-[#00ff88] focus:ring-0 cursor-pointer"
            />
        ...
        <div className="flex items-center gap-3 pt-3 mt-3 border-t border-[#1a3a24]">
