...
const statusConfig = {
  ...
  finalizada: { label: 'Finalizada', className: 'text-text-muted bg-gray-500/10 border-gray-500/30'          },
...
const MetricSelectorModal = React.memo(function MetricSelectorModal({ campaign, allOptions, onSave, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-lg bg-background border border-border rounded-xl shadow-2xl flex flex-col max-h-[85vh]">
            ...
            {allOptions.map(opt => {
              ...
              return (
                <button key={opt.key} onClick={() => toggle(opt.key)}
                  className={`flex items-center justify-between px-3 py-2.5 rounded-xl border text-left text-xs font-medium transition-all ${ativo ? 'bg-emerald-500/15 border-emerald-500/40 text-emerald-300' : 'bg-[#121a15] border-border text-text-muted hover:text-text-main hover:border-border'}`}>
                  <span>{opt.label}</span>
        ...
        <div className="px-6 py-4 border-t border-border flex gap-3">
          ...
          <button onClick={handleSave} disabled={saving || selected.length === 0}
            className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-text-main text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors">
...
const CampaignCard = React.memo(function CampaignCard({ campaign, fetchMetrics, fetchAllMetricOptions, saveSelectedMetrics, dateStart, dateEnd }: {
  ...
  return (
    <>
      <div className="bg-[#121a15] border border-border rounded-xl overflow-hidden hover:border-emerald-500/30 transition-all shadow-xl">
        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-emerald-500/10 text-emerald-400 flex items-center justify-center shadow-inner">
              <Megaphone size={22} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={abrirConfig} className="w-8 h-8 rounded-xl bg-background border border-border flex items-center justify-center text-text-muted hover:text-emerald-400 hover:border-emerald-500/30 transition-all" title="Configurar métricas">
              <Settings2 size={14} />
            </button>
            <button onClick={loadMetrics} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${expandido ? 'bg-emerald-600 text-white' : 'bg-background border border-border text-text-muted hover:text-text-main'}`}>
              <BarChart2 size={14} />
          ...
          <div className="px-5 pb-5 pt-2 border-t border-border bg-background/50">
            {loadingMetrics ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {metrics.map((m) => (
                  <div key={m.id} className="bg-[#121a15] border border-border rounded-xl p-3 shadow-sm">
...
export default function ColaboradorCampanhasPage() {
  ...
  return (
    <div className="p-8 space-y-8 pb-20 bg-background">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button onClick={handleSync} disabled={loading}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${loading ? 'bg-emerald-500/20 text-emerald-400' : 'bg-[#121a15] border border-border text-text-main hover:text-text-main'}`}>
      ...
      <div className="sticky top-0 z-30 flex flex-wrap items-center gap-4 bg-[#121a15]/95 backdrop-blur-md p-3 rounded-xl border border-border shadow-lg">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Buscar campanha pelo nome..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full pl-12 pr-4 py-2.5 bg-background border border-border rounded-xl text-text-main text-sm placeholder-gray-600 focus:outline-none focus:border-emerald-500/50 transition-all" />
        </div>
  <div className="flex items-center gap-2">
    ...
    {[{
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-background border border-border rounded-xl">
          ...
          <input type="date" value={dateStart} onChange={e => setDateStart(e.target.value)} className="bg-transparent text-text-main text-sm focus:outline-none" style={{ colorScheme: 'dark' }} />
          <span className="text-text-disabled">—</span>
          <input type="date" value={dateEnd} onChange={e => setDateEnd(e.target.value)} className="bg-transparent text-text-main text-sm focus:outline-none" style={{ colorScheme: 'dark' }} />
        </div>
      </div>
      ...
      <div className="space-y-10">
        {loading && campaigns.length === 0 ? (
          ...
          <div className="bg-[#121a15] border border-border rounded-xl p-20 text-center">
          ...
          <>
            ...
            {ativos.length > 0 && (
              <div className="space-y-4">
                <h2 className="text-emerald-400 text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
            ...
            {bloqueados.length > 0 && (
              <>
                ...
                <div className="space-y-4">
                  <h2 className="text-red-400 text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
