...
export default function ColaboradorAlertasPage() {
  ...
  const [formData, setFormData] = useState<AlertInput>({
    nome: '', tipo: 'saldo_minimo', canal: 'meta', conta_anuncio: '', saldo_minimo: null,
    recebedor_tipo: 'privado', recebedor_numero: '', mensagem_template: '', ativo: true
  })
            ...
            <div key={alert.id} className="bg-surface border border-border p-6 rounded-xl flex flex-col justify-between hover:border-primary/40 transition-colors shadow-sm">
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  {alert.canal === 'meta' ? <Facebook size={18} className="text-primary" /> : <Globe size={18} className="text-red-500" />}
                </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="grid gap-2">
                    <button type="button" onClick={() => setFormData({...formData, canal: 'meta'})} className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm ${formData.canal === 'meta' ? 'bg-primary/10 border-primary/40 text-primary' : 'bg-background border-border text-text-muted'}`}><Facebook size={14} /> Meta Ads</button>
                    <button type="button" onClick={() => setFormData({...formData, canal: 'google'})} className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm ${formData.canal === 'google' ? 'bg-red-50 border-red-300 text-red-600' : 'bg-background border-border text-text-muted'}`}><Globe size={14} /> Google Ads</button>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-1">
                    {(formData.tipo === 'saldo_minimo' ? ['<CA>', '<SALDO>', '<TARGET>'] : ['<CA>', '<ACT_STATUS>']).map(v => (
