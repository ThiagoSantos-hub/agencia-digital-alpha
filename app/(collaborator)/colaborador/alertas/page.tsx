...
import { Plus, Bell, Facebook, Globe, Trash2, Edit2, Copy, Smartphone, Users, AlertTriangle, DollarSign, Wallet, CheckCircle2, X, Save, Loader2, Search } from 'lucide-react'
...
const inputVazio: AlertInput = {
  nome: '', tipo: 'saldo_minimo', canal: 'meta', conta_anuncio: '', saldo_minimo: 0,
...
export default function ColaboradorAlertasPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      if (editingAlert) await updateAlerta(editingAlert.id, formData)
      else await createAlerta(formData)
  ...
  const handleDuplicate = async (alert: Alert) => {
    const { id, user_id, created_at, updated_at, creator, client, last_status, ...rest } = alert
    try { await createAlerta({ ...rest, nome: `${rest.nome} (Cópia)` }) } catch (e) { console.error(e) }
  }
  ...
  const insertVariable = (variable: string) => {
    setFormData(prev => ({ ...prev, mensagem_template: prev.mensagem_template + variable }))
  }
    ...
    <div className="min-h-full text-text-main">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2 text-text-main">
            <Bell className="text-primary" /> Alertas
          </h1>
          <p className="text-text-muted text-sm">Configure notificações automáticas para suas contas</p>
        </div>
        <button onClick={() => handleOpenModal()} className="bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded-xl flex items-center gap-2 font-medium shadow-sm">
      ...
      {viewMode === 'tabela' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {[...Array(3)].map((_, i) => <div key={i} className="bg-surface border border-border h-48 rounded-xl animate-pulse" />)}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-surface border border-border rounded-xl shadow-sm">
          ...
          <button onClick={() => handleOpenModal()} className="mt-4 text-primary hover:underline text-sm font-medium">Criar meu primeiro alerta</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {sortedAlerts.map((alert) => {
            ...
            <div key={alert.id} className="bg-surface border border-border p-6 rounded-xl flex flex-col justify-between hover:border-primary/40 transition-colors shadow-sm">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {alert.tipo === 'saldo_minimo' ? (
                    <span className="bg-amber-50 text-amber-700 text-[10px] font-bold uppercase px-2 py-1 rounded-md flex items-center gap-1"><DollarSign size={10} /> Saldo Mínimo</span>
                  ) : alert.tipo === 'erro_conta' ? (
                    <span className="bg-red-50 text-red-600 text-[10px] font-bold uppercase px-2 py-1 rounded-md flex items-center gap-1"><AlertTriangle size={10} /> Erro na Conta</span>
                  ) : (
                    <span className="bg-cta/10 text-cta text-[10px] font-bold uppercase px-2 py-1 rounded-md flex items-center gap-1"><Wallet size={10} /> Fundo de Cliente</span>
                  )}
                  {vencido && (
                    <span className="bg-red-50 text-red-600 text-[10px] font-bold uppercase px-2 py-1 rounded-md">Vencido</span>
                  )}
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-border flex justify-between items-center">
                <button onClick={() => toggleAtivo(alert.id, !alert.ativo)} className={`w-10 h-5 rounded-full relative transition-colors ${alert.ativo ? 'bg-cta' : 'bg-slate-300'}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setDeletingAlert(alert)} className="p-1.5 hover:bg-red-50 rounded-lg text-text-muted hover:text-red-500"><Trash2 size={16} /></button>
                </div>
              </div>
            </div>
            )
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface border border-border w-full max-w-lg rounded-xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-border flex justify-between items-center">
              <h2 className="text-xl font-bold text-text-main">{editingAlert ? 'Editar Alerta' : 'Criar Alerta'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-hover-bg rounded-lg text-text-muted"><X size={20} /></button>
            </div>
            <form onSubmit={handleSave} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-text-muted uppercase">Nome do Alerta</label>
                <input required type="text" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} className={inputCls} placeholder="Ex: Alerta de Saldo - Alpha" />
              </div>
              <div className="space-y-1.5">
              ...
              {formData.tipo === 'fundo_cliente' ? (
                <>
                  <div className="space-y-1.5">
                    ...
                    <select required value={formData.client_id ?? ''} onChange={e => handleSelecionarCliente(e.target.value)} className={`${inputCls} appearance-none`}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input required type="number" step="0.01" min="0" value={formData.valor_fundo ?? ''} onChange={e => setFormData({...formData, valor_fundo: parseFloat(e.target.value) || 0})} className={inputCls} placeholder="500.00" />
                    </div>
                    <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {(['semanal', 'quinzenal', 'mensal', 'anual'] as const).map(p => (
                        <button key={p} type="button" onClick={() => setFormData({...formData, periodicidade: p})} className={`py-2 rounded-xl border text-xs font-medium transition-colors ${formData.periodicidade === p ? 'border-primary/40 text-primary bg-primary/10' : 'border-border text-text-muted'}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input required type="text" value={formData.conta_anuncio} onChange={e => setFormData({...formData, conta_anuncio: e.target.value})} className={inputCls} placeholder="act_123456789" />
                  </div>
                  {formData.tipo === 'saldo_minimo' && (
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-text-muted uppercase">Saldo Mínimo (R$)</label>
                      <input required type="number" value={formData.saldo_minimo || ''} onChange={e => setFormData({...formData, saldo_minimo: Number(e.target.value)})} className={inputCls} placeholder="100.00" />
                    </div>
                  )}
                </div>
              )}
              ...
              {formData.tipo !== 'fundo_cliente' && (
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setFormData({...formData, canal: 'meta'})} className={`flex items-center justify-center gap-2 px-3 py-2 rounded-xl border text-sm ${formData.canal === 'meta' ? 'bg-primary/10 border-primary/40 text-primary' : 'bg-background border-border text-text-muted'}`}><Facebook size={14} /> Meta Ads</button>
                    <button type="button" onClick={() => setFormData({...formData, canal: 'google'})} className={`flex items-center justify-center gap-2 px-3 py-2 rounded-xl border text-sm ${formData.canal === 'google' ? 'bg-red-50 border-red-300 text-red-600' : 'bg-background border-border text-text-muted'}`}><Globe size={14} /> Google Ads</button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-text-muted uppercase">Template</label>
                ...
                <textarea required value={formData.mensagem_template} onChange={e => setFormData({...formData, mensagem_template: e.target.value})} className={`${inputCls} min-h-[100px] resize-none`} />
              </div>
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 border border-border hover:bg-hover-bg text-text-muted py-2.5 rounded-xl text-sm font-medium">Cancelar</button>
                <button type="submit" disabled={isSaving} className="flex-1 bg-primary hover:bg-primary-hover text-white py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50">
                  {isSaving ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} Salvar Alerta
                </button>
              </div>
            </form>
          </div>
