    ...
    <div className="p-8 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Gerencie suas receitas e gastos de forma simples.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-[#0a0f0c] border border-[#1a3a24] p-6 rounded-2xl">
          ...
          <p className="text-gray-400 text-sm font-medium">Total Gastos</p>
        ...
        <div className="bg-[#0a0f0c] border border-[#1a3a24] p-6 rounded-2xl">
          ...
          <h3 className={`text-2xl font-bold mt-1 ${saldo >= 0 ? 'text-white' : 'text-red-400'}`}>
      ...
      <div className="bg-[#0a0f0c] border border-[#1a3a24] rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#1a3a24] bg-[#1a3a24]/20">
            ...
            <tbody className="divide-y divide-[#1a3a24]">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500 italic">Carregando lançamentos...</td>
                </tr>
              ) : finances.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500 italic">Nenhum lançamento encontrado.</td>
                </tr>
              ) : (
                finances.map((item) => (
                  <tr key={item.id} className="hover:bg-[#1a3a24]/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-400">{formatDate(item.date)}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0a0f0c] border border-[#1a3a24] w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[#1a3a24] flex items-center justify-between">
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <select 
                  ...
                  className="w-full bg-[#1a3a24]/20 border border-[#1a3a24] rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-all"
                >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#1a3a24]/20 border border-[#1a3a24] rounded-xl pl-9 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-all"
                    />
                  </div>
                </div>
              ...
              <div className="pt-4 flex gap-3">
                <button 
                  ...
                  className="flex-1 px-4 py-2.5 border border-[#1a3a24] text-gray-400 font-bold rounded-xl hover:bg-[#1a3a24]/20 transition-all"
                >
                  Cancelar
                </button>
                <button 
