    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
            ...
            {(['agencia', 'pessoal'] as EscopoFinanceiro[]).map(e => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${escopoAtivo === e
                  ...
                  : 'text-text-muted border border-border hover:border-[#00ff88]/30 hover:text-text-main'}`}
              >
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm border border-border text-text-muted hover:border-[#00ff88]/30 hover:text-text-main transition-colors ml-auto"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Card Receitas */}
        <div className="bg-[#0d1510] border border-border rounded-xl p-6 shadow-xl">
      ...
      <div className="flex gap-6">
        <div className="flex-1 bg-[#0d1510] border border-border rounded-xl overflow-hidden">

          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            <div className="flex items-center gap-3">
              <button onClick={() => navMes(-1)} className="w-7 h-7 rounded-lg border border-border flex items-center justify-center text-text-muted hover:text-text-main hover:border-[#00ff88]/40 transition-colors">
                <ChevronLeft size={14} />
              </button>
          ...
          <div className="flex border-b border-border">
            {([['todas', 'Todas'], ['receitas', 'Receitas'], ['despesas', 'Despesas']] as [AbaFiltro, string][]).map(([v, label]) => (
              <button key={v} onClick={() => setAbaFiltro(v as AbaFiltro)}
                className={`flex-1 py-2.5 text-sm font-medium transition-colors ${abaFiltro === v ? 'text-[#00ff88] border-b-2 border-[#00ff88]' : 'text-text-muted hover:text-text-main'}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="px-5 py-3 border-b border-border">
            <div className="relative">
              ...
              <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Pesquisar por descrição ou categoria..."
                className="w-full bg-background border border-border rounded-xl pl-8 pr-4 py-2 text-sm text-text-main placeholder-gray-600 focus:outline-none focus:border-[#00ff88]/40" />
            </div>
          </div>
          ...
          <div className="divide-y divide-[#1a3a24]">
            {loading ? (
              ...
              Object.entries(agrupados).map(([categoria, items]) => {
                ...
                return (
                  <div key={categoria}>
                    <div className="flex items-center justify-between px-5 py-3 bg-background/60">
                      <div className="flex items-center gap-2">
                    ...
                    {items.map(l => (
                      <div key={l.id} className={`flex items-center justify-between px-5 py-3.5 hover:bg-hover-bg transition-colors border-l-2 ${l.tipo === 'receita' ? 'border-l-[#00ff88]/40' : l.tipo === 'investimento' ? 'border-l-blue-500/40' : 'border-l-red-500/40'}`}>
                        ...
                        <div className="flex items-center gap-4 ml-4">
                          ...
                          <div className="flex items-center gap-1">
                            {l.status !== 'pago' && (
                              <button onClick={() => marcarComoPago(l.id)} className="w-7 h-7 rounded-lg flex items-center justify-center text-text-disabled hover:text-[#00ff88] hover:bg-[#00ff88]/10 transition-colors" title="Marcar como pago">
                            ...
                            <button onClick={() => setDeletandoId(l.id)} className="w-7 h-7 rounded-lg flex items-center justify-center text-text-disabled hover:text-red-400 hover:bg-red-500/10 transition-colors">
                              <Trash2 size={13} />
                            </button>
                          </div>
                        </div>
        ...
        <div className="w-72 flex-shrink-0">
          <div className="bg-[#0d1510] border border-border rounded-xl p-5 sticky top-6">
      ...
      {modalAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-md bg-[#0d1510] border border-border rounded-xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5 custom-scrollbar">
              ...
              <div>
                ...
                <input type="number" min="0" step="0.01" value={formInput.valor || ''} onChange={e => setFormInput(p => ({ ...p, valor: parseFloat(e.target.value) || 0 }))} placeholder="R$ 0,00"
                  className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-text-main text-sm placeholder-gray-700 focus:outline-none focus:border-[#00ff88]/40" />
              </div>
              <div>
                  ...
                  <div className="flex gap-2">
                    {(['gasto', 'investimento'] as TipoLancamento[]).map(t => (
                      <button key={t} onClick={() => setFormInput(p => ({ ...p, tipo: t, categoria: '' }))}
                        className={`flex-1 py-2 rounded-xl text-xs font-medium border transition-colors ${formInput.tipo === t ? 'border-[#00ff88]/40 text-[#00ff88] bg-[#00ff88]/10' : 'border-border text-text-muted'}`}>
              ...
              <div>
                ...
                <select value={formInput.categoria} onChange={e => setFormInput(p => ({ ...p, categoria: e.target.value }))}
                  className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-text-main text-sm focus:outline-none focus:border-[#00ff88]/40 appearance-none">
              ...
              <div className="flex items-center justify-between bg-background border border-border rounded-xl px-4 py-3">
                ...
                <button onClick={() => setEditStatus(s => s === 'pendente' ? 'pago' : 'pendente')}
                  className={`w-11 h-6 rounded-full transition-colors relative flex items-center px-1 ${editStatus === 'pago' ? 'bg-[#00ff88]' : 'bg-hover-bg'}`}>
              ...
              <div className="flex items-center justify-between bg-background border border-border rounded-xl px-4 py-3">
                ...
                <button onClick={() => setFormInput(p => ({ ...p, recorrente: !p.recorrente }))}
                  className={`w-11 h-6 rounded-full transition-colors relative flex items-center px-1 ${formInput.recorrente ? 'bg-[#00ff88]' : 'bg-hover-bg'}`}>
                  ...
                  <div className="flex gap-2">
                    {(['mensal', 'semanal', 'anual'] as const).map(r => (
                      <button key={r} onClick={() => setFormInput(p => ({ ...p, recorrencia: r }))}
                        className={`flex-1 py-2 rounded-xl text-xs font-medium border transition-colors ${formInput.recorrencia === r ? 'border-[#00ff88]/40 text-[#00ff88] bg-[#00ff88]/10' : 'border-border text-text-muted'}`}>
              ...
              <div className="flex items-center gap-2 bg-background border border-border rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-text-main text-xs font-medium">Pessoa Responsável</p>
                  <p className="text-text-disabled text-xs">Thiago Santos</p>
                </div>
              </div>
            </div>

            <div className="flex-shrink-0 px-6 py-5 border-t border-border bg-[#0d1510]/95 backdrop-blur-sm z-20">
              <div className="flex flex-col gap-2.5">
                <button onClick={handleSalvar} disabled={salvando}
                  className={`w-full py-4 rounded-xl text-sm font-bold shadow-xl transition-all active:scale-[0.96] disabled:opacity-50 ${modalTipo === 'receita' ? 'bg-[#00ff88] text-[#0a0f0c] hover:bg-[#00dd77] shadow-[#00ff88]/20' : 'bg-red-500 text-text-main hover:bg-red-600 shadow-red-500/20'}`}>
      ...
      {deletandoId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-sm bg-[#0d1510] border border-red-500/30 rounded-xl p-6 shadow-2xl">
