          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!isFree && (
              <div>
                ...
                {paymentMethod === 'card' ? (
                  <p className="text-xs text-text-muted mt-2">Assinatura mensal recorrente, cobrada automaticamente todo mês no cartão cadastrado.</p>
                ) : (
                  <p className="text-xs text-text-muted mt-2">Pagamento único via Pix (com acréscimo de 10% sobre o valor do cartão), libera 30 dias de acesso. Pra continuar usando depois, é só pagar de novo dentro do sistema.</p>
                )}
              </div>
            )}
