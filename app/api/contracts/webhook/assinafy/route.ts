...
// Formato do payload de webhook da Assinafy — confirmar contra a documentação atual
// antes de ativar em produção (nomes de evento/campos podem variar), mesmo cuidado já
// sinalizado em lib/esignature/assinafy.ts.
...
export async function POST(request: NextRequest) {
  try {
    const payload: AssinafyWebhookPayload = await request.json()
    ...
    const secret = await getAssinafyWebhookSecret(contract.company_id)
    const provided = request.headers.get('x-assinafy-webhook-secret') ?? new URL(request.url).searchParams.get('secret')
    ...
    const clientSigner = payload.data.signers?.find((s) => s.email === contract.email)
    const companySigner = payload.data.signers?.find((s) => s.email === companySignerEmail)

    const signerClientStatus = clientSigner?.status === 'signed' ? 'assinado' : contract.signer_client_status
    const signerCompanyStatus = companySigner?.status === 'signed' ? 'assinado' : contract.signer_company_status
    ...
    const signedFileUrl = payload.data.signed_file_url
    ...
    console.error('Erro no webhook de contratos (Assinafy):', error)
