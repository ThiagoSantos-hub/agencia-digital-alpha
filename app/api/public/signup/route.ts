...
export async function POST(request: Request) {
  try {
    const { companyName, adminName, adminEmail, phone, facebookProfile, paymentMethod } = await request.json()
    ...
    const metadata = { companyName, adminName, adminEmail, phone, facebookProfile, paymentMethod }
    ...
    // preço do cartão + 10%, libera 30 dias, sem trial.
    const basePrice = await stripe.prices.retrieve(process.env.STRIPE_PRICE_ID!)
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          product_data: { name: 'Assinatura Digital Alpha — 1 mês (Pix)' },
