...
export function useAlphaAI(): UseAlphaAIReturn {
  ...
  const chamarAPI = useCallback(async (texto: string, incluirVoz: boolean): Promise<void> => {
    ...
    const msgUsuario: ChatMessage = {
      id: gerarId(), role: 'user', content: texto, createdAt: new Date(),
    }
    ...
    try {
      ...
      const msgAlpha: ChatMessage = {
        ...
        content: data.resposta ?? '', audio: data.audio ?? null, createdAt: new Date(),
      }
  ...
  const sendAudio = useCallback(async (blob: Blob, mimeType: string): Promise<void> => {
    if (loading) return
    setLoading(true)
    setError(null)
